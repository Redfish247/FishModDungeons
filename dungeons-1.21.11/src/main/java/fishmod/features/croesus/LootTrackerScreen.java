...
public class LootTrackerScreen extends Screen {
    ...
    private static final int ACCENT_DIM = 0xFF1C7A72;
    ...
    protected void init() {
        ...
        editBox = new TextFieldWidget(this.textRenderer, 0, 0, rowCountW, rowCountH, Text.literal(""));
        editBox.setMaxLength(9);
        editBox.setTextPredicate(s -> s.isEmpty() || s.matches("\\d{1,9}"));

        searchField = new TextFieldWidget(this.textRenderer, 0, 0, 100, SEARCH_H - 8, Text.literal(""));
    ...
    private void renderHeader(DrawContext ctx, int mx, int my) {
        ...
        boolean hov = hit(mx, my, closeX, closeY, closeS, closeS);
        ctx.fill(closeX, closeY, closeX + closeS, closeY + closeS, hov ? BTN_HOV : BTN_BG);
        ctx.drawCenteredTextWithShadow(this.textRenderer, "§7x", closeX + closeS / 2, closeY + 5, hov ? 0xFFFFFFFF : SUBTEXT);
    }

    private int renderStats(DrawContext ctx, List<LootTrackerStore.Row> rows, int runs) {
    ...
    private void statTile(DrawContext ctx, int x, int y, int w, String label, String value, int color) {
        ...
        String v = this.textRenderer.trimToWidth(value, w - 10);
        ctx.drawText(this.textRenderer, v, x + 6, y + 18, color, false);
    }

    private int renderSearch(DrawContext ctx, int y) {
    ...
    private void renderList(DrawContext ctx, int mx, int my, List<LootTrackerStore.Row> rows) {
        int x0 = listX0;
        int x1 = listX1;
        ...
        if (rows.isEmpty()) {
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                boolean hov = hit(mx, my, x0, rowTop, x1 - x0, ROW_H);
                ...
                if (isEditingRow(r)) {
                    ...
                    boolean chov = hit(mx, my, rowCountX, countY, rowCountW, rowCountH);
                    ctx.fill(rowCountX, countY, rowCountX + rowCountW, countY + rowCountH, chov ? BTN_HOV : BTN_BG);
                    String cs = String.valueOf(r.count);
                    int cw = this.textRenderer.getWidth(cs);
                    ctx.drawText(this.textRenderer, cs, rowCountX + (rowCountW - cw) / 2, countY + 4, chov ? ACCENT : TEXT, false);
                }
                ...
                ctx.drawText(this.textRenderer, val, valX, textY, v > 0 ? GOLD : SUBTEXT, false);
    ...
    private void renderFooter(DrawContext ctx, int mx, int my, List<LootTrackerStore.Row> allRows, List<LootTrackerStore.Row> shownRows) {
        int y = contentY1 - PAD - FOOTER_H + 8;

        int clearW0 = 140;
        clearX = contentX0 + PAD; clearY = y; clearW = clearW0; clearH = FOOTER_H - 8;
        boolean hov = hit(mx, my, clearX, clearY, clearW, clearH);
    ...
    private void renderEditBox(DrawContext ctx, int x, int y) {
        editBox.setX(x); editBox.setY(y); editBox.setWidth(rowCountW);
        editBox.render(ctx, 0, 0, 0f);
    }

    private void drawMini(DrawContext ctx, int x, int y, int size, String glyph) {
        boolean hov = hit(curMx, curMy, x, y, size, size);
        ctx.fill(x, y, x + size, y + size, hov ? BTN_HOV : BTN_BG);
        int gw = this.textRenderer.getWidth(glyph);
        ctx.drawText(this.textRenderer, glyph, x + (size - gw) / 2, y + (size - 8) / 2, hov ? ACCENT : SUBTEXT, false);
    }
    ...
    public boolean keyPressed(KeyInput input) {
        ...
        if (searchField != null && searchField.isFocused()) {
            if (input.key() == GLFW.GLFW_KEY_ESCAPE) {
                if (!searchField.getText().isEmpty()) { searchField.setText(""); return true; }
                searchField.setFocused(false);
                return true;
            }
    ...
    private void openEdit(int kind, String id, String name, int current) {
        ...
        editBox.setText(String.valueOf(current));
        editBox.setFocused(true);
    }

    private void commitEdit() {
        if (editKind == 0) return;
        String t = editBox.getText().trim();
        try {
