...
object Section {
    ...
    const val SPLIT_LENGTH: Int = 120
    ...
    @JvmStatic
    fun parseMessage(message: Text): Boolean {
        ...
        if (matcher.find()) {
            ...
            if (Floor7.terminalTimeStamps) {
                ...
                if (texts.isNotEmpty()) {
                    Misc.addChatMessage(
                        Text.literal(name).setStyle(texts.first().style)
                            .append(Text.literal(" §a$action $objective! (§c$currentCompleted§a/$totalNeeded) §8(§7${getSectionTime()}s §8| §7${Phase.getPhaseTime(TERM_PHASE_INDEX)}s§8)"))
                    )
