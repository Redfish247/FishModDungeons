...
object Reputation {
    ...
    private fun pollFlags(mc: MinecraftClient) {
        val handler = mc.networkHandler ?: return
        val uuidToName = HashMap<String, String>()
        for (e in handler.playerList) {
            val gp = e.profile
            if (gp.name.isBlank()) continue
     ...
     */
    @JvmStatic
    fun decorateTab(text: Text?): Text? {
    ...
    private fun withUuid(name: String, cb: (String?) -> Unit) {
        ...
        if (cached != null) { cb(cached.replace("-", "")); return }
        HypixelApi.resolveUuidAsync(name) { uuid -> cb(if (uuid == null) null else uuid.replace("-", "")) }
    }
    ...
    fun vote(name: String, dir: String) {
        ...
        val voter = mc.player!!.uuid.toString().replace("-", "")
        Misc.addChatMessage(Text.literal("§7[Rep] resolving §f$name§7…"))
        withUuid(name) { uuid ->
            if (uuid == null) {
                mc.execute { Misc.addChatMessage(Text.literal("§c[Rep] couldn't find player §f$name")) }
                return@withUuid
            }
            HypixelApi.voteRep(voter, uuid, name, dir) { up, down ->
                mc.execute {
                    if (up < 0) {
                        Misc.addChatMessage(Text.literal("§c[Rep] vote failed — worker route not reachable."))
                        return@execute
                    }
    ...
    fun listNearby() {
        ...
        if (handler == null) { Misc.addChatMessage(Text.literal("§c[Rep] Not in a world.")); return }
        val uuidToName = HashMap<String, String>()
        for (e in handler.playerList) {
            val gp = e.profile
            if (gp.name.isBlank()) continue
        ...
        HypixelApi.fetchReps(uuidToName.keys) { reps ->
            mc.execute {
                ...
                if (flagged.isEmpty()) { Misc.addChatMessage(Text.literal("§a[Rep] No flagged players in this lobby.")); return@execute }
                Misc.addChatMessage(Text.literal("§c[Rep] §l" + flagged.size + " flagged player(s) here:"))
                for (line in flagged) Misc.addChatMessage(Text.literal("  $line"))
            }
        }
    }
