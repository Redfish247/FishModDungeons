...
class CreditsScreen(private val parent: Screen?) : Screen(Text.literal("Credits")) {
    ...
    override fun renderBackground(ctx: DrawContext, mx: Int, my: Int, d: Float) {}
    ...
    override fun render(ctx: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        ctx.drawCenteredTextWithShadow(
            this.textRenderer,
            Text.literal((if (linkHov) "§b" else "§9") + DISCORD), cx, linkY + 5, DISCORD_BLURPLE
        )
        ...
        ctx.drawCenteredTextWithShadow(this.textRenderer, Text.literal("Back"), cx, backY + (backH - 8) / 2, 0xFF052A29.toInt())

        super.render(ctx, mouseX, mouseY, delta)
    }

    private fun drawCredit(ctx: DrawContext, x: Int, y: Int, name: String, role: String) {
    ...
    override fun mouseClicked(click: Click, bl: Boolean): Boolean {
        ...
        if (inside(mx, my, linkX, linkY, linkW, linkH)) {
            try {
                Util.getOperatingSystem().open(DISCORD_URL)
