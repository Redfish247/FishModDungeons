 ...
 */
class ItemCustomizeScreen : Screen(Text.literal("Item Customize")) {
    ...
    private lateinit var modelField: TextFieldWidget
    private lateinit var dyeField: TextFieldWidget
    private lateinit var skinField: TextFieldWidget
    ...
    private fun inv(): PlayerInventory = client!!.player!!.inventory
    private fun mainCount(): Int = Math.min(36, inv().size())
    ...
    override fun init() {
        ...
        for (i in 0 until mainCount()) if (inv().getStack(i) === held) { selectedIndex = i; break }
        ...
        nameField = TextFieldWidget(this.textRenderer, fx, nameY, fw, 14, Text.literal("Name"))
        nameField.setMaxLength(128)
        addDrawableChild(nameField)

        modelField = TextFieldWidget(this.textRenderer, fx, modelY, fw, 14, Text.literal("Model"))
        modelField.setMaxLength(64)
        addDrawableChild(modelField)

        // Dye: hex box on the right, dropdown on the left (built below).
        dyeField = TextFieldWidget(this.textRenderer, fx + 156, dyeY, fw - 156, 14, Text.literal("Hex"))
        ...
        dyeDropdown.onChange = Runnable {
            if (dyeDropdown.selected >= 0) dyeField.text = DYES[dyeDropdown.selected][1]
        }
        ...
        skinField = TextFieldWidget(this.textRenderer, fx, skinY, fw, 14, Text.literal("Skin"))
        ...
        addDrawableChild(
            ButtonWidget.builder(Text.literal("Apply")) { _ -> apply() }
                .dimensions(btnX, btnY, btnW, 20).build()
        )
        addDrawableChild(
            ButtonWidget.builder(Text.literal("§eReset")) { _ -> reset() }
        ...
        addDrawableChild(
            ButtonWidget.builder(Text.literal("Done")) { _ -> close() }
                .dimensions(btnX + (btnW + 6) * 2, btnY, btnW, 20).build()
        )
        addDrawableChild(
            ButtonWidget.builder(Text.literal("§cClear All")) { _ -> ItemCustomizer.clearAll(); loadFields() }
    ...
    private fun loadFields() {
        val sel = inv().getStack(selectedIndex)
        val c = ItemCustomizer.get(sel)

        // Name + Model pre-fill with the item's BASE values so the user edits, not starts blank.
        nameField.text = if (c != null && !c.name().isNullOrEmpty()) c.name() else sel.name.string
        val baseModel = ItemCustomizer.vanillaId(sel)
        modelField.text = if (c != null && !c.modelId().isNullOrEmpty()) c.modelId() else (baseModel ?: "")

        val dye = c?.dye() ?: -1
        dyeField.text = if (dye >= 0) String.format("%06X", dye and 0xFFFFFF) else ""
    ...
    private fun apply() {
        ...
        val skin = if (isHead(sel)) skinField.text.trim() else ""
    ...
    private fun dyeAllowed(st: ItemStack?): Boolean {
        ...
        return try { st.contains(DataComponentTypes.DYED_COLOR) } catch (e: Exception) { false }
    }

    private fun insertIntoName(code: String) {
        val t = nameField.text
        val cur = Math.min(nameField.cursor, t.length)
    ...
    private fun idOf(st: ItemStack): String {
        try {
            val cd = st.get(DataComponentTypes.CUSTOM_DATA)
            if (cd != null) {
                val id = cd.copyNbt().getString("id", "")
                if (id.isNotEmpty()) return id
            }
        } catch (ignored: Exception) {
        }
        return Registries.ITEM.getId(st.item).toString()
    }
    ...
    private fun drawPanel(ctx: DrawContext, mouseX: Int, mouseY: Int) {
        val sel = inv().getStack(selectedIndex)
        val p = panelX + 14
        ...
        ctx.drawTextWithShadow(this.textRenderer, sel.name, p + 46, curY, TEXT_PRIM)
        ...
        val prevY = skinY + 28
        val nameTxt = nameField.text
        ctx.drawTextWithShadow(this.textRenderer, "§7Preview:", p, prevY, TEXT_HINT)
        ctx.drawTextWithShadow(this.textRenderer, fishmod.cosmetic.NickState.parse(nameTxt), fx, prevY, 0xFFFFFFFF.toInt())
    }

    private fun drawLabel(ctx: DrawContext, s: String, x: Int, y: Int) {
    ...
    private fun drawColorKey(ctx: DrawContext, mouseX: Int, mouseY: Int) {
        keyRects.clear()
        keyCodes.clear()
        ...
        for (i in CODE_COLORS.indices) {
            ...
            ctx.drawCenteredTextWithShadow(this.textRenderer, code.toString(), colX + sw / 2, colY + 2, textCol)
        ...
        for (f in CODE_FORMATS) {
            val sample = f[1]
            val w = this.textRenderer.getWidth(sample) + 6
            val hov = inBox(mouseX, mouseY, fxr, fyr, w, sh)
            ctx.fill(fxr, fyr, fxr + w, fyr + sh, if (hov) ROW_HOVER else BG_FIELD)
            ctx.drawTextWithShadow(this.textRenderer, sample, fxr + 3, fyr + 2, 0xFFFFFFFF.toInt())
        ...
        val starW = this.textRenderer.getWidth("&* ✪") + 8
        ...
        ctx.drawTextWithShadow(this.textRenderer, "§7&* §6✪", starX + 4, starY + 2, 0xFFFFFFFF.toInt())
    ...
    private inner class Dropdown(val placeholder: String, val x: Int, val y: Int, val w: Int) {
        ...
        fun renderClosed(ctx: DrawContext, mx: Int, my: Int) {
        ...
        fun renderOpen(ctx: DrawContext, mx: Int, my: Int) {
            if (!open) return
            val rows = rowsShown()
            val ly = y + boxH
            val lh = rows * rowH
            ...
            for (r in 0 until rows) {
                ...
                ctx.drawTextWithShadow(textRenderer, clip(labels[idx], w - (tx - x) - 4), tx, ry + 3, col)
            }
        ...
        private fun clip(s: String, maxW: Int): String {
            ...
            var out = s
            while (out.length > 1 && textRenderer.getWidth("$out…") > maxW) out = out.substring(0, out.length - 1)
            return "$out…"
        }
    }
