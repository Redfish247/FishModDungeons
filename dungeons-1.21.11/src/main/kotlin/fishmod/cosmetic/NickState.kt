    ...
    fun parse(input: String?): Text {
        ...
        while (i < input.length) {
            ...
            if ((c == '&' || c == '§') && i + 1 < input.length) {
                ...
                val fmt = Formatting.byCode(next.lowercaseChar())
                if (fmt != null) {
                    if (buf.isNotEmpty()) {
                        root.append(Text.literal(buf.toString()).setStyle(style))
                        buf.setLength(0)
                    }
                    style = if (fmt == Formatting.RESET) {
                        Style.EMPTY
                    } else if (fmt.isColor) {
                        Style.EMPTY.withColor(fmt)
                    } else {
                        style.withFormatting(fmt)
                    }
                    i += 2
                    continue
                }
