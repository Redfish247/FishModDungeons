...
object GradientNick {
    ...
    fun parseColor(token: String?): IntArray? {
        ...
        var f: Formatting? = null
        if (t.length == 1) f = Formatting.byCode(t[0])
        if (f == null) f = Formatting.byName(t.uppercase())
        if (f != null && f.isColor && f.colorValue != null) return rgb(f.colorValue!!)
        return null
    }
