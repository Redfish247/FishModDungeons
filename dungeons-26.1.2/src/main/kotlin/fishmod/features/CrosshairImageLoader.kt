...
object CrosshairImageLoader {
    ...
    private val IMAGES_PATH: Path = FabricLoader.getInstance().configDir.resolve("FishMod").resolve("crosshairs")
    ...
    fun init() {
        ...
        try {
            ...
            val t = Thread(::watchLoop, "FishMod-CrosshairImageLoader")
            t.isDaemon = true
            t.start()
        ...
        try {
            Files.newInputStream(path).use { input ->
                ...
                Minecraft.getInstance().execute {
                    ...
                    Misc.addChatMessage(Component.literal("§a[Crosshair] Loaded image: §f$fileName §7(${w}x$h)"))
                }
            }
        } catch (e: Exception) {
    ...
    @JvmStatic
    fun getImageSize(name: String): IntArray? = LOADED[name]?.let { intArrayOf(it.width, it.height) }
    ...
    private data class ImageData(val id: Identifier, val texture: DynamicTexture, val width: Int, val height: Int)
}
