...
object DeskBuddy {
    ...
    private fun tick(mc: Minecraft) {
        ...
        if (!poseInit) {
            pX = p.x; pY = p.y; pZ = p.z; pYaw = p.yRot; pPitch = p.xRot
            poseInit = true
            lastActivityMs = now
        } else {
            val moved = p.x != pX || p.y != pY || p.z != pZ || p.yRot != pYaw || p.xRot != pPitch
    ...
    fun renderHud(ctx: GuiGraphicsExtractor, tickCounter: DeltaTracker) {
        ...
        val lh = fishmod.utils.Constants.TEXT_HEIGHT + 1
        ctx.pose().pushMatrix()
        ctx.pose().translate(x.toFloat(), y.toFloat())
        ctx.pose().scale(sc, sc)
        ctx.text(mc.font, "§6$name", 0, 0, 0xFFFFFFFF.toInt(), true)
        ctx.text(mc.font, faceStr, wiggle.roundToInt(), lh + bob.roundToInt(), 0xFFFFFFFF.toInt(), true)
        ctx.text(mc.font, moodStr, 0, lh * 2 + 2, 0xFFFFFFFF.toInt(), true)
        ctx.pose().popMatrix()
    }
}
