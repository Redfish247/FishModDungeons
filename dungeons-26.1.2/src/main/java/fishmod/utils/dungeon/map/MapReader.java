...
public class MapReader {
    ...
    private static void sample(byte[] colors) {
        ...
        // been sampled yet at that point in iteration order (it's later in the same row or in a
        // future row), so thisType == neighborType could never be true and merging silently never
        // fired. Splitting into two passes fixes that.
     ...
     */
    private static void updateDoorOpenedState(Minecraft mc) {
        for (var entry : DungeonGrid.allDoors().entrySet()) {
            ...
            if (mc.level.isLoaded(pos) && mc.level.getBlockState(pos).isAir()) {
    ...
    private static void updatePlayerMarkers(Minecraft mc, MapItemSavedData map) {
     ...
     * {@code WorldScan.getRoomCore}: scans straight down at a FIXED offset (+7, +7) from the room
     * tile's northwest corner — not the room's true center — building a string of block registry
     * names from the first non-air/gold block down to bedrock, then hashes it and looks the hash up
     * in the vendored {@link RoomData} database. The +7 offset must match exactly, since it's the
     * same position Odin's own data-collection scanned when building the cores in that database —
     * scanning anywhere else in the room produces a different (non-matching) hash. (An earlier
     * version of this method scanned the true center, +16, which silently never matched anything.)
     * {@code Block.toString()} returns the registry name deterministically, so the hash itself is
     * stable across game sessions/machines — confirmed via javap before relying on it, not assumed.
     */
    private static void identifyOneRoom(Minecraft mc) {
        for (RoomTile tile : DungeonGrid.allRooms().values()) {
            ...
            if (!mc.level.isLoaded(new BlockPos(centerX, 70, centerZ))) continue;

            tile.scanAttempted = true;
            int core = getRoomCore(mc.level, centerX, centerZ);
    ...
    private static int getRoomCore(net.minecraft.world.level.Level level, int x, int z) {
        ...
        for (int y = 140; y >= 12; y--) {
            ...
            if (!foundHighest) {
                if (!state.isAir() && state.getBlock() != net.minecraft.world.level.block.Blocks.GOLD_BLOCK) {
                    foundHighest = true;
                } else {
                    sb.append('0');
                    continue;
            ...
            if (state.getBlock() == net.minecraft.world.level.block.Blocks.BEDROCK) {
                ...
                if (state.getBlock() == net.minecraft.world.level.block.Blocks.OAK_PLANKS
                        || state.getBlock() == net.minecraft.world.level.block.Blocks.TRAPPED_CHEST
